...
import {
    ...
    FaCommentAlt, FaPaperPlane, FaCheckDouble
} from 'react-icons/fa';
...
const HireDriver = () => {
    ...
    useEffect(() => {
        ...
        const register = () => {
            ...
            if (activeJobRef.current?._id) socket.emit('join_job_room', activeJobRef.current._id);
        };
        ...
        socket.on('job_completed', (data) => {
            ...
            if (activeJobRef.current && activeJobRef.current._id === data.requestId) {
        ...
        socket.on('new_job_message', (message) => {
            ...
            if (isChatOpen && message.senderId !== (currentUser?._id || currentUser?.uid) && activeJobRef.current) {
                socket.emit('mark_messages_seen', { requestId: activeJobRef.current._id, readerId: currentUser._id || currentUser.uid });
            }
        });
    ...
    const fetchRecommendations = useCallback(async (lat, lon) => {
        ...
        try {
            const res = await fetch(`${API_BASE_URL}/api/recommend/search`, {
                ...
                credentials: 'include',
                body: JSON.stringify({
                    user_id: currentUser?._id || currentUser?.uid || 'guest',
                    lat,
                    lon,
                    radius_km: 50,
                    top_n: 20,

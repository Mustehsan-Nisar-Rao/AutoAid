...
const ForgotPassword = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                success('Password reset OTP sent! Please check your inbox.');
                navigate('/reset-password', { state: { email } });
            } else {
                setEmailError(data.error || 'Failed to send reset OTP.');
            }
        } catch (err) {
            console.error('Error sending password reset OTP:', err);
            setEmailError('Failed to send reset OTP. Please try again.');
        }
    };
                    ...
                    <div className="w-full lg:w-1/2 flex items-center justify-center p-4">
                        <div className="w-full max-w-md p-8 space-y-6 bg-white dark:bg-card-dark rounded-lg shadow-xl border border-gray-200 dark:border-border-dark">
                            <div className="text-center">
                                ...
                                <p className="mt-2 text-gray-600 dark:text-subtle-dark">Enter your email to receive a reset OTP.</p>
                            </div>

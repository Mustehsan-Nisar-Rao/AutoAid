...
const ProviderActiveJob = () => {
  ...
  useEffect(() => {
    ...
    socketRef.current.on('connect', () => {
        socketRef.current.emit('register_provider', currentUser._id);
    ...
    socketRef.current.on('new_job_message', (message) => {
        ...
        if (isChatOpen && message.senderId !== (currentUser._id || currentUser.uid)) {
            socketRef.current.emit('mark_messages_seen', { requestId: job._id, readerId: currentUser._id });
        }
    });

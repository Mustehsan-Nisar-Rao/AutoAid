...
// Helper to safely find user by ID without throwing Mongoose CastErrors for non-ObjectId strings
const safeFindUserById = async (id) => {
    ...
    try {
        ...
        return await User.findOne({ _id: String(id) });
    } catch (e) {
        return null;
    }
};

// Helper to safely update user by ID
const safeUpdateUserById = async (id, update) => {
    if (!id) return null;
    try {
        if (mongoose.Types.ObjectId.isValid(id)) {
            return await User.findByIdAndUpdate(id, update, { new: true });
        }
        return await User.findOneAndUpdate({ _id: String(id) }, update, { new: true });
    } catch (e) {
        return null;
    }
};
...
exports.createServiceRequest = async (req, res) => {
    try {
        const { userId, serviceType, contactNumber, details, userLocation } = req.body;

        if (!userId || !serviceType || !contactNumber || !details) {
...
exports.assignProvider = async (req, res) => {
    try {
        ...
        // Build update object
        const updateObj = { providerId: String(providerId) };
        ...
        const user = await safeFindUserById(request.userId);
        ...
        if (io && connectedProviders) {
            const provider = await safeFindUserById(providerId);
            if (provider) {
                const socketId = connectedProviders.get(provider._id.toString());
                if (socketId) {
...
exports.sendCounterOffer = async (req, res) => {
    try {
        ...
        if (!counterRate || isNaN(counterRate) || Number(counterRate) <= 0) {
        ...
        if (!request.providerId || String(request.providerId) !== String(providerId)) {
        ...
        if (io && connectedUsers) {
            const userSocketId = connectedUsers.get(String(request.userId));
            if (userSocketId) {
                const provider = await safeFindUserById(providerId);
    ...
    try {
        ...
        if (action === 'accept') {
            ...
            if (request.providerId) {
                await safeUpdateUserById(request.providerId, { isAvailable: false });
            }

            // Notify provider
            if (io && connectedProviders) {
                const provider = await safeFindUserById(request.providerId);
                if (provider) {
                    const providerSocketId = connectedProviders.get(provider._id.toString());
                    if (providerSocketId) {
            ...
            // Re-enable provider
            if (request.providerId) {
                await safeUpdateUserById(request.providerId, { isAvailable: true });
            }
...
exports.getProviderRequests = async (req, res) => {
    try {
        ...
        const populatedRequests = await Promise.all(requests.map(async (reqItem) => {
            const user = await safeFindUserById(reqItem.userId);
...
exports.getActiveJob = async (req, res) => {
    try {
        ...
        const user = await safeFindUserById(activeRequest.userId);
...
exports.getUserActiveJob = async (req, res) => {
    try {
        ...
        const provider = await safeFindUserById(activeRequest.providerId);
        const populatedRequest = {
            ...activeRequest._doc,
...
exports.updateProviderLocation = async (req, res) => {
    try {
        ...
        const updatedProvider = await safeUpdateUserById(
            providerId,
...
exports.updateRequestStatus = async (req, res) => {
    try {
        ...
        if (status === 'Accepted') {
            ...
            await safeUpdateUserById(request.providerId, { 
                isAvailable: true,
